...
- [ ] Task 2 — Make the preview window correctly implement all signal processing using the same signal processing pipeline as the live camera feed.
- [ ] Task 6 — Add a "Crop" slider into the feed controls. Crops the processed image as the final processing step before display by setting pixels outside the crop region to black (not resizing). Exempt those pixels from "zero-referenced display" calculations.
...
## Group I — Fixes to Camera Controls Buttons

**Status: [ ] Not started**
**Primary files:** `src/Windows/CameraControls.py` (buttons)

Tasks (from TODO.md, in priority order):

- [ ] Task 22 — Make the buttons background be the correct color (#252526). Make the "Save" button be half-height instead of half-width.
