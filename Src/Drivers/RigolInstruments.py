...
class RigolDG4162(object):
	...
	def getVoltage(self, channel):
		...
		voltage = self.resource.query(command)
		try:
	...
	def getOutputState(self, channel):
		"""
		Get the output state (ON/OFF) of the specified channel.
	...
	def setOutputState(self, channel, state):
		if type(state != bool):
			print('Error: state must be a boolean')
			return
		
		command = ':OUTP' + str(channel) + (' ON' if state else ' OFF')
		self.resource.write(command)
